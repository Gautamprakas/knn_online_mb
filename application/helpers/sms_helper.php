...
{  $url="http://bhashsms.com/api/sendmsg.php?user=pranimation&pass=mgfhbl123&sender=PARAMN&phone=".$number."&text=".urlencode($msg)."&priority=ndnd&stype=normal";
...
function email( $toMail , $info , $type){
   ...
   require_once 'application/libraries/phpmailer/mail.php';
   ...
   if(!empty($subject)){
   	 try{
   	 	return myMail2($sendFrom,$password,$senderName,$sendTo,$receiverName,$subject,$body);
   	 }catch(Exception $e){
...
function getStartEndDate(){
	$start_date = isset($_GET['start_date']) ? $_GET['start_date'] : "2021-01-01";
